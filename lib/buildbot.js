...
Buildbot.prototype.getBuilders = function(callback){
  var self = this;
  var url = self.url("/json/builders");
  request.get({uri: url}, function(error, response, body){
    if(error){
      ...
      var builders = [];
      for(var builderName in bodyJson){
        builders.push(self.builder(builderName));
      }
      callback(null, builders, body);
    }
  });
};
